...
import { apartment, apartments } from "../../../../../lib/services/types";
...
export const GalleryCard = ({ media, apartments }: apartment) => {
  ...
  return (
    <div className="flex flex-col relative   w-full lg:max-w-sm  bg-white items-start justify-center">
      <Swiper
        ...
        autoplay
        navigation={{
          prevEl: `.custom-swiper-button-prev-${apartments?.id}`,
          nextEl: `.custom-swiper-button-next-${apartments?.id}`,
        }}
          ...
          <div key={idx} className="relative w-full h-full">
            <SwiperSlide className=" z-10 absolute  h-full" key={idx}>
              <Image
                width={900}
                height={900}
                src={`${SERVER_URL}/${image.filePath}`}
                alt=" "
                className=" absolute h-full   object-cover"
              />
              <div className="relative opacity-0 hover:opacity-100 transition duration-150 w-full h-full ">
                <div
                  className={` cursor-pointer   absolute top-[45%]  text-primery p-2 rounded-full m-2 bg-white custom-swiper-button-prev-${apartments?.id}`}
                >
                  <FaArrowLeft />
                </div>
                <div
                  className={` cursor-pointer absolute right-0 top-[45%] text-primery p-2 rounded-full m-2 bg-white custom-swiper-button-next-${apartments?.id}`}
                >
                  <FaArrowRight />
                </div>
              </div>
      ...
      <div className="absolute  w-full  bottom-[259px] ">
        ...
        <div className="  z-30 w-full p-2  top-10  absolute ">
          <h2 className=" text-white font-normal text-base mt-2">
            {apartments?.title}
          </h2>
      ...
      <Button
        fill={false}
        hover={true}
        onClick={() => router.push(`/houseDetails/${apartments?.id}`)}
      >
        view details
      </Button>
    </div>
